...
const loadData = async () => {
 // 页面加载后的逻辑
  ...
  stats.value[0].value =resp.today.amount;//金额
  stats.value[0].diff =resp.change_percent.amount;//百分比
  stats.value[1].value =resp.today.quantity;//数量
  stats.value[1].diff =resp.change_percent.quantity;//百分比
}
</script>
