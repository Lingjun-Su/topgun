    ...
    >
      <template v-slot:top-left>
        <q-btn-group outline>
          ...
          <q-btn color="negative" icon="delete_sweep" label="逻辑删除" :disable="!selected.length" @click="confirmDelete" />
        </q-btn-group>
      </template>
...
const allColumns = [
  ...
  { name: 'bus_code', label: '业务标识', field: 'bus_code', align: 'left' },
  { name: 'sku_code', label: '产品标识', field: 'sku_code', align: 'left' },
...
// 默认可见列
const DEFAULT_COLUMNS = ['order_no', 'user_phone', 'bus_code', 'total_amount', 'sync_status', 'created_at']
...
const filterModel = ref({ bus_id: null, sku_code: null })
const filters = ref({
  order_no: '',
  user_phone: '',
  sync_status: null
...
//   $q.loading.show({ message: '正在推送...' })
//   productOrderApi.push(row.id).then(() => {
//     $q.notify({ type: 'positive', message: '单条推送成功' })
//     fetchData()
//   }).finally(() => $q.loading.hide())
// }

// const getSyncLabel = (val) => syncOptions.find(o => o.value === val)?.label || '未知'
// const getSyncColor = (val) => ['orange', 'green', 'red'][val] || 'grey'

// const resetFilters = () => {
//   filters.value = { order_no: '', user_phone: '', sync_status: null }
//   filterModel.value = { bus_id: null, sku_code: null }
//   fetchData()
// }
