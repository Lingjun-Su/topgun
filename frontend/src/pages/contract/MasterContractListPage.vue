...
const fetchList = async () => {
  ...
  try {
    ...
    // 后端 data 是分页器对象（data.data 才是行数组），直接赋给 rows 会导致 q-table 渲染崩溃、loading 卡死
    rows.value = data?.data || []
    // 同步分页信息（总条数用于分页展示）
    if (data && typeof data === 'object') {
      pagination.value.page = data.current_page ?? pagination.value.page
      pagination.value.rowsPerPage = data.per_page ?? pagination.value.rowsPerPage
      pagination.value.rowsNumber = data.total ?? pagination.value.rowsNumber
    }
  } catch (error) {
