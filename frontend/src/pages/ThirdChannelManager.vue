      ...
      <q-card style="min-width: 500px">
        ...
        <q-card-section>
          <q-form @submit="saveChannel" class="q-gutter-md">
            ...
            <organization-tree-select v-model="form.organization_id" label="归属组织" />
...
const manageProducts = async (row) => {
  ...
  productDialog.items = res.data.data.products || []
...
const saveProductConfig = async () => {
  try {
    ...
    $q.notify({ color: 'positive', message: '产品配置更新成功' })
...
const addProductToConfig = () => {
  ...
  if (exists) {
    $q.notify({ type: 'warning', message: '该产品已在配置列表中' })
    return
  }
