...
const routes = [
  // 第一组路由：根路径（/）的主布局+首页规则
  {
    ...
    children: [// 子路由规则：匹配根路径（/）的默认子路由
      ...
      {path:'user-info',component:()=>import('pages/users/UserInfoPage.vue'),meta:{title:"用户信息",requiresAuth: true},name:"用户信息"},
      {path:'dictionary',component:()=>import('pages/DictionaryManager.vue'),meta:{title:"数据字典",requiresAuth: true},name:"数据字典"},
      {path:'excel-import',component:()=>import('pages/ExcelImportPage.vue'),meta:{title:"Excel导入",requiresAuth: true},name:"Excel导入"},
      {path:'third-channel',component:()=>import('pages/ThirdChannelManagerPage.vue'),meta:{title:"渠道接入管理",requiresAuth: true},name:"渠道接入管理"},
      ...
      {path:'order-product',component:()=>import('pages/ProductManagerPage.vue'),meta:{title:"产品管理",requiresAuth: true},name:"产品管理"},
      ...
      {path:'user-list',component:()=>import('pages/UserManagerPage.vue'),meta:{title:"用户管理",requiresAuth: true},name:"用户管理"},
      ...
      {path:'receiver-test',component:()=>import('pages/receiverTestPage.vue'),meta:{title:"接收测试",requiresAuth: true},name:"接收测试"},

      //数据中转管理（旧推送队列路由已随后端 T-11 删除，页面迁移至 pages/dataforward/）
      {path:'forward-orders',component:()=>import('pages/dataforward/ForwardOrderPage.vue'),meta:{title:"数据中转管理",requiresAuth: true},name:"数据中转管理"},
      {path:'forward-stats',component:()=>import('pages/dataforward/ForwardStatsPage.vue'),meta:{title:"订单错误码统计",requiresAuth: true},name:"订单错误码统计"},
      {path:'push-stats',component:()=>import('pages/dataforward/PushStatsPage.vue'),meta:{title:"推送统计",requiresAuth: true},name:"推送统计"},
      {path:'callback-logs',component:()=>import('pages/dataforward/CallbackLogPage.vue'),meta:{title:"回调日志",requiresAuth: true},name:"回调日志"},
      {path:'callback-simulator',component:()=>import('pages/dataforward/CallbackSimulatorPage.vue'),meta:{title:"回调模拟器",requiresAuth: true},name:"回调模拟器"},
      {path:'forward-stop-logs',component:()=>import('pages/dataforward/ForwardStopLogPage.vue'),meta:{title:"停止信息查询",requiresAuth: true},name:"停止信息查询"},
      ...
      //合同
      {path:'master-contract',component:()=>import('pages/contract/MasterContractListPage.vue'),meta:{title:'框架合同列表',requiresAuth:true},name:'框架合同列表'},
