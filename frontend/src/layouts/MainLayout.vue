...
const menuList = computed(() => {
  ...
  const allowedPaths = new Set(dp.page_restrictions.map(p => '/' + p))

  // 只保留子菜单链接在 allowedPaths 中的菜单项
  return fullMenuList
    .map(group => {
      const filteredSub = (group.sub || []).filter(sub => allowedPaths.has(sub.link))
...
const submitPassword = async () => {
  ...
  try {
    // 密码修改通过 HTTPS 传输保证安全（见 quasar.config.js API_URL 配置）
    // 生产环境请确保使用 HTTPS 地址，避免密码明文被中间人截获
