...
class MasterContractStateMachine
{
     ...
     * 统一执行状态转换（验证 + 转换 + 保存）
     *
     * 调用示例: $machine->execute('submit')
     *
     * @param  string  $action  动作名 (submit/withdraw/approve/reject/void/terminate/addClause)
     * @return int 转换后的新状态
     *
     * @throws InvalidArgumentException 当动作不允许或转换失败时
     */
    public function execute(string $action): int
    {
        // 1. 验证动作在当前状态下是否允许
        if (! $this->isActionAllowedByState($action)) {
            throw new InvalidArgumentException(
                sprintf('当前状态 %d 不允许执行 "%s" 操作', $this->contract->status, $action)
            );
        }

        // 2. 执行动作，获取目标状态
        $newStatus = $this->$action(); // 调用 submit()/withdraw()/approve() 等方法
