...
abstract class BaseProductOrder extends Model implements Auditable
{
    ...
    protected $fillable = [
        ...
        'quantity', 'total_amount', 'sync_status', 'sync_error', 'pushed_at', 'ext_json', 'internal_amount',
        'trace_id', 'product_id', 'business_id', 'channel_id', 'organization_id',
        'forward_log', 'link_id',
    ];
    ...
    {
        // 1. 基础查询
        $query = ProductOrder::query()->whereIn('order_status', [1, 2]); // 仅统计首次订购+继订中，排除未付款和退订
    ...
    public static function getSalesPieCharts()
    {
        ...
        $otherQuery = ProductOrder::query()
            ->whereIn('order_status', [1, 2]) // 保持与主查询一致
            ->whereBetween('order_time', [$startDate, $endDate]);
