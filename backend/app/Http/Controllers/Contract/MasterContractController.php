...
class MasterContractController extends Controller
{
    ...
    private const STATUS_TERMINATED = 5;
    private const STATUS_VOID = 6;
    ...
    public function update(UpdateMasterContractRequest $request, MasterContract $contract)
    {
        ...
        // 非草稿且非驳回状态时，禁止修改核心字段
        if (!in_array($contract->status, [self::STATUS_DRAFT, self::STATUS_REJECTED])) {
    ...
    public function terminate(MasterContract $contract)
    {
        ...
        if (!$contract->stateMachine()->isActionAllowedByState('terminate')) {
            return $this->error('当前状态不允许终止合同', 400);
        }

        $newStatus = $contract->stateMachine()->terminate();
        $contract->status = $newStatus;
        $contract->save();
