    ...
    public function up(): void
    {
        ...
        foreach ($tables as $tableName) {
            Schema::create($tableName, function (Blueprint $table) {
                ...
                // --- 索引优化 ---
                // 既然下方有了 pid + order_no 的唯一索引，这里可以根据需要保留或调整其他索引
                $table->index(['user_phone', 'order_no'], 'idx_phone_order');
                $table->index(['created_at', 'sync_status'], 'idx_created_sync');
                $table->index('pid', 'idx_pid'); // 增加 pid 单独索引提高查询效率
            });
