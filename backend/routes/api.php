...
use App\Http\Controllers\API\ForwardStatsController;
...
use App\Http\Controllers\API\ThirdChannel\ForwardStopController;
...
Route::middleware(['auth:sanctum', 'data.permission'])->prefix('v1')->group(function () {
    ...
    // 渠道配置管理
    Route::get('channels/stop-condition-types', [ThirdChannelController::class, 'stopConditionTypes']);
    ...
    // ---------停止条件记录查询 (ForwardStop)-------
    Route::prefix('forward-stop-logs')->group(function () {
        Route::get('/', [ForwardStopController::class, 'index']);
        Route::get('{id}', [ForwardStopController::class, 'show']);
    });
    ...
    // ---------验证码流程按天错误码统计 (ForwardStats)-------
    Route::prefix('forward-stats')->group(function () {
        Route::get('daily', [ForwardStatsController::class, 'daily']);
        Route::get('error-codes', [ForwardStatsController::class, 'errorCodes']);
    });
